...
public:
	...
	void setBinDelayAmps(const int a, const float b) {
		par::ampArray[a] = b;
		interpolateSurroundingIndices(par::ampArray, MainVar::numBands, a, b, 5);
...
private:
	...
	float panSpeed[MainVar::numBands];
	float panLocation[MainVar::numBands];
	...
	void interpolateSurroundingIndices(float* array, int arraySize, int index, float value, int nval) {
		...
		for (int i = -nval; i < nval; i++) {
			if (i + index >= 0 && i + index < MainVar::numBands) {
				if (i != 0) {
