...
typedef struct {
   ...
   * has been color converted and readBuffer.
 ...
 * simple replication of the input pixel onto the corresponding readBuffer
	      ...
	      JSAMPARRAY input_data, JSAMPARRAY * output_data_ptr)
{
  ...
  while (outrow < cinfo->max_v_samp_factor) {
    /* Generate one readBuffer row with proper horizontal expansion */
	...
	*outptr++ = invalue;
    ...
    /* Generate any additional readBuffer rows by duplicating the first one */
    if (v_expand > 1) {
 ...
 * speed and visual quality.  The centers of the readBuffer pixels are 1/4 and 3/4
