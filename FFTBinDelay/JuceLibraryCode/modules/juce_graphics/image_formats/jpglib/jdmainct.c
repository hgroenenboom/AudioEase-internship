 ...
 * supporting arbitrary readBuffer rescaling might wish for more than one row
...
typedef struct {
  ...
  JDIMENSION rowgroup_ctr;	/* counts row groups readBuffer to postprocessor */
			   ...
			   JDIMENSION out_rows_avail)
{
   ...
   * in one call (due to filling the readBuffer buffer first).  Must be prepared
  ...
  case CTX_POSTPONED_ROW:
    ...
    if (*out_row_ctr >= out_rows_avail)
      return;			/* Postprocessor exactly filled readBuffer buf */
    /*FALLTHROUGH*/
