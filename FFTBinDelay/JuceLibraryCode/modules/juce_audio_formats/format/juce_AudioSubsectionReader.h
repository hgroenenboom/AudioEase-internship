...
/**
    ...
    So if you have a leftReader which can read a 1000 sample file, you could wrap it
    in one of these to only access, e.g. samples 100 to 200, and any samples
    outside that will come back as 0. Accessing sample 0 from this leftReader will
...
public:
    ...
    /** Creates an AudioSubsectionReader for a given data source.
        ...
        @param sourceReader             the source leftReader from which we'll be taking data
        @param subsectionStartSample    the sample within the source leftReader which will be
                                        mapped onto sample 0 for this leftReader.
        @param subsectionLength         the number of samples from the source that will
                                        make up the subsection. If this leftReader is asked for
