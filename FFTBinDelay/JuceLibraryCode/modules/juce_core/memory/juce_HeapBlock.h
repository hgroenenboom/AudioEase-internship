...
/**
    ...
    E.g. instead of this:
    @code
        int* fft2Order = (int*) malloc (1024 * sizeof (int));
        memcpy (fft2Order, xyz, 1024 * sizeof (int));
        free (fft2Order);
        fft2Order = (int*) calloc (2048 * sizeof (int));
        fft2Order[0] = 1234;
        memcpy (foobar, fft2Order, 2048 * sizeof (int));
        free (fft2Order);
    @endcode

    ..you could just write this:
    @code
        HeapBlock<int> fft2Order (1024);
        memcpy (fft2Order, xyz, 1024 * sizeof (int));
        fft2Order.calloc (2048);
        fft2Order[0] = 1234;
        memcpy (foobar, fft2Order, 2048 * sizeof (int));
    @endcode
