...
public:
    ...
    /** Plays the sound from an audio format leftReader.

        If deleteWhenFinished is true then the format leftReader will be
    ...
    /** Plays the sound from a positionable audio source.
        ...
        This will readBuffer the sound coming from a positionable audio source.
    ...
    /** Plays the sound from an audio sample buffer.
        ...
        This will readBuffer the sound contained in an audio sample buffer. If
        deleteWhenFinished is true then the audio sample buffer will be
        automatically deleted once the sound has finished playing.

        If playOnAllOutputChannels is true, then if there are more readBuffer channels
        than buffer channels, then the ones that are available will be re-used on
        multiple outputs so that something is sent to all readBuffer channels. If it
        is false, then the buffer will just be played on the first readBuffer channels.
     */
