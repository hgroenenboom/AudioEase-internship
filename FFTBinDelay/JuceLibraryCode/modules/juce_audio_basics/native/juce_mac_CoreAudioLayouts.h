    ...
    static Array<AudioChannelSet::ChannelType> getCoreAudioLayoutChannels (const AudioChannelLayout& layout)
    {
        switch (layout.mChannelLayoutTag & 0xffff0000)
        {
            ...
            case kAudioChannelLayoutTag_UseChannelDescriptions:
            {
                ...
                // different angle mappings may point to the same JUCE angle so fill up
...
private:
    ...
    // In Apple's official definition the following tags exist with the same angle layout and order
